SELECT s.sbs_no AS "Subsidiary No"
	...
	, nvl(ai.orig_value, 0) AS "Original Qty"
	, nvl(ai.adj_value, 0) AS "Adjusted Qty"
	, nvl(ai.adj_value, 0) - nvl(ai.orig_value, 0) AS "Qty Difference"
	, nvl(ai.cost, 0) * nvl(ai.orig_value, 0) AS "Total Original Cost"
	, nvl(ai.cost, 0) * nvl(ai.adj_value, 0) AS "Total Adjusted Cost"
	, (nvl(ai.adj_value, 0) - nvl(ai.orig_value, 0)) * nvl(ai.cost, 0) AS "Total Cost Difference"
