...
| Version | Highlights |
|---|---|
| 3.1.2 (2026-07-30) | Slicer bars aligned app-wide: every filter box (Business Partner, Account, Item, Customer, Document type, quick-search…) now carries the caption-above style, and preset chips / toggles / switches sit on the same baseline as the boxes |
| 3.1.1 (2026-07-30) | Field labels render above the box app-wide (no more label/border overlap), one caption color token everywhere; Users dialog: page-access catalog filtered by the license, first-field label clip fixed; repo cleanup: Electron remnants, legacy Krunch queries and stale scripts removed; full handover documentation (5 volumes, Word + single-file HTML) in docs/ |
