...
async def background_warmer():
    ...
    while True:
        ...
        host      = cfg.get("host", "<ORACLE-SERVER>")
        it        = "(1,2)"
...
def get_subsidiaries(host: str = "<ORACLE-SERVER>"):
    try:
...
@app.get("/api/stores")
def get_stores(host: str = "<ORACLE-SERVER>", subsidiary_sid: Optional[str] = None):
    try:
